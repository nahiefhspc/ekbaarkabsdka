...
async def start_command(client: Client, message: Message):
    ...
    if len(text) > 7:
        ...
        if link_type == "HACKHEIST":
            ...
            for msg in messages:
                ...
                caption = (
                    get_config(client, 'CUSTOM_CAPTION', CUSTOM_CAPTION).format(
                        previouscaption=(msg.caption.html if msg.caption else "🔥 𝐇𝐈𝐃𝐃𝐄𝐍𝐒 🔥"),
                        filename=filename,
                        mediatype=media_type,
                    )
                    if bool(get_config(client, 'CUSTOM_CAPTION', CUSTOM_CAPTION))
                ...
                try:
                    copied_msg = await msg.copy(
                        ...
                        protect_content=get_config(client, 'PROTECT_CONTENT', PROTECT_CONTENT),
                    )
                    if copied_msg:
            ...
            for msg in messages:
                ...
                individual_button = InlineKeyboardButton("😁 𝗖𝗟𝗜𝗖𝗞 𝗧𝗢 𝗦𝗔𝗩𝗘 📥", url=f"https://t.me/{getattr(client, 'username', 'bot')}?start={base64_string2}")
    ...
    # Default /start
    start_msg = get_config(client, 'START_MSG', START_MSG)
...
async def not_joined(client: Client, message: Message):
    ...
    # Safe username handling for clones
    ...
    force_msg = get_config(client, 'FORCE_MSG', FORCE_MSG)
...
# Baaki saare commands (users, add_random_message, broadcast etc.) same hain jaise original file mein the
# (broadcast, scheduled broadcast, resume etc. sab yahin hain - space bachane ke liye yahan nahi likha, lekin original file mein hain)
...
# Add your other commands (add_random_message, broadcast, broadcast_add, etc.) below if needed
# I kept them as per your original request
